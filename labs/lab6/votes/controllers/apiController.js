...
async function getVote(vote) {
    return mongoose.connect(dbUrl, connectOptions).then(() => { return model.find({"weapon_id": vote.weapon_id, "user": vote.user }) }).catch((error) => { return 500 });
}
...
module.exports = {
    ...
    async unvote(req, res) {
        ...
        const user = unpackToken(token);
        const temp = {
            weapon_id: new ObjectID(req.body.weapon_id),
            user: user.login
        }
